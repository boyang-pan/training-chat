...
import { Plus, Trash2, Pencil, MoreHorizontal, PanelLeftClose, Settings, Search, Pin, Keyboard, ChevronUp, LogOut, Sun, Moon, Monitor } from "lucide-react";
...
function UserAvatar({ name, email }: { name?: string | null; email?: string | null }) {
  ...
  return (
    <span className="w-6 h-6 rounded-full bg-zinc-800 dark:bg-zinc-200 text-white dark:text-zinc-900 text-[11px] font-semibold flex items-center justify-center shrink-0 select-none">
      {initial}
    </span>
  );
}
...
export function Sidebar({ conversations, isLoadingConversations, activeId, onSelect, onNew, onDelete, onRename, userEmail, userName, onLogout, onOpenModal, onOpenSearch, onOpenShortcuts, onPin }: SidebarProps) {
    ...
    <div className="border-r border-zinc-100 dark:border-zinc-800 flex flex-col h-full bg-white dark:bg-zinc-900 overflow-hidden">
      ...
      {userEmail && (
        <div className="px-2 py-2 border-t border-zinc-100 dark:border-zinc-800">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="w-full flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors group">
                <UserAvatar name={userName} email={userEmail} />
                <span className="text-xs text-zinc-600 dark:text-zinc-300 truncate flex-1 min-w-0 text-left">
                  {userName ?? userEmail}
                </span>
                <ChevronUp className="w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="top" align="start" className="w-56 mb-1">
              <DropdownMenuLabel className="text-xs font-normal text-zinc-500 dark:text-zinc-400 truncate py-1.5">
                {userEmail}
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
              ...
              <DropdownMenuItem onClick={onOpenShortcuts}>
                <Keyboard className="w-3.5 h-3.5 mr-2 text-zinc-500" />
                Keyboard shortcuts
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => setTheme(THEME_CYCLE[(theme ?? "system") as keyof typeof THEME_CYCLE])}
              >
                {(() => { const Icon = THEME_ICON[(theme ?? "system") as keyof typeof THEME_ICON] ?? Monitor; return <Icon className="w-3.5 h-3.5 mr-2 text-zinc-500" />; })()}
                {THEME_LABEL[(theme ?? "system") as keyof typeof THEME_LABEL] ?? "System"} theme
              </DropdownMenuItem>
              <DropdownMenuSeparator />
